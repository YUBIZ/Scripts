...
public static void GenerateSiteMap(string sourceDirPath, string outputFilePath, string baseUrl, string[] searchPatterns, string[] excludePatterns)
{
    ...
    var urlSet = filePathAndCommitHistoryCollection.Select(v => new Url(baseUrl + Uri.EscapeDataString(v.FilePath), v.CommitHistory.Max(v1 => v1.Date).ToString("yyyy-MM-dd"), null, null)).ToArray();
